...
//add by stevenrao
static int64_t timeout_val = 0;

static void timeout_handler(int signum){
   fprintf(stderr, "adb exec time out");
   exit(1);
}
...
static bool wait_for_device(const char* service,
    ...
    if (components[3] != "any" && components[3] != "bootloader" && components[3] != "device" &&
        ...
        components[3] != "disconnect" && components[3] != "reset") {
        fprintf(stderr,
                "adb: unknown state %s; "
...
int adb_commandline(int argc, const char** argv) {
    ...
    while (argc > 0) {
        if (!strcmp(argv[0], "server")) {
            ...
            argc--;
            argv++;
        } else if ( !strncmp(argv[0], "-T", 2) ) { //add by stevenrao
            const char* timeout_str;
            if (isdigit(argv[0][2])) {
                timeout_str = argv[0] + 2;
            } else {
                timeout_str = argv[1];
                argc--;
                argv++;
            }
            timeout_val = strtoll(timeout_str, const_cast<char**>(&timeout_str), 10);
            if (*timeout_str != '\0') {
    ...
    //add by stevenrao
    if( timeout_val > 0 ) {
        signal(SIGALRM, timeout_handler); // Register signal handler
        alarm( timeout_val );
    }
    //add end
    ...
    if (!strcmp(argv[0], "devices")) {
        ...
        if (argc > 2) error_exit("usage: adb reset serial");
        std::string query = android::base::StringPrintf("host:reset:%s",argv[1]);
	return adb_query_command(query);
    } 
