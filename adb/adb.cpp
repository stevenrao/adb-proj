...
HostRequestResult handle_host_request(std::string_view service, TransportType type,
    ...
    //add by stevenrao 
    if (service.starts_with("reset:")) {
       std::string serial = std::string(service.substr(6));
       atransport* t = find_transport(serial.c_str());
       if (t == nullptr) {
           SendFail(reply_fd, android::base::StringPrintf("no such device '%s'", serial.c_str()));
           return HostRequestResult::Handled;
	   ...
	   kick_transport(t);
	   LOG(INFO)  <<  "usb_reset serial " << serial << " should_use_libusb() " <<  should_use_libusb() ;
	   /*
       usb_handle* h =  t->GetUsbHandle();
       if( h ){
           usb_reset(h);   
       }
	   */

       SendOkay(reply_fd, android::base::StringPrintf("reset %s", serial.c_str()));      
       return HostRequestResult::Handled;
    }
    //add end
